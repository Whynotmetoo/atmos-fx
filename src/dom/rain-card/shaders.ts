...
void main() {
  ...
  float packed     = floor(water.b * 255.0 + 0.5);
  float thickness  = (floor(packed / ${HIGHLIGHT_AREA_LEVELS}.0) / ${DROP_DEPTH_LEVELS - 1}.0) * ${MAX_DROP_DEPTH};
  float hlLevel    = mod(packed, ${HIGHLIGHT_AREA_LEVELS}.0) / ${HIGHLIGHT_AREA_LEVELS - 1}.0;
