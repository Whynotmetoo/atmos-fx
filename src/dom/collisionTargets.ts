...
export function collectCollisionTargetRects(root: HTMLElement): CollisionTargetRect[] {
  const rootRect = root.getBoundingClientRect()
  const targets: CollisionTargetRect[] = []

  for (const target of root.querySelectorAll(COLLISION_SELECTOR)) {
...
export function createCollisionTargetManager(
  ...
  const refresh = () => {
    ...
    scheduled = false
    targets = collectCollisionTargetRects(root)
